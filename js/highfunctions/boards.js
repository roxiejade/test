...
function openCompose(mode, threadId, type) {
  ...
  document.getElementById('bv2-img-preview').style.display = 'none';
  document.getElementById('bv2-img-preview-src').src = '';
...
function handleImgSelect(e) {
  ...
  const previewImg = document.getElementById('bv2-img-preview-src');
  const previewWrap = document.getElementById('bv2-img-preview');
...
function removeSelectedImage() {
  ...
  // 检查表情包预览是否还有内容
  const stickerPreview = document.getElementById('bv2-sticker-preview');
  const previewArea = document.getElementById('bv2-preview-area');
  if ((!stickerPreview || stickerPreview.children.length === 0 || stickerPreview.style.display === 'none') && previewArea) {
...
window._bv2_doMultiSelect = function(action) {
  ...
  window._bv2_removeSelectedImage = removeSelectedImage;
window._bv2_openStickerPicker = openStickerPickerForCompose;
...
// 打开表情包选择器
function openStickerPickerForCompose() {
    const stickers = (typeof stickerLibrary !== 'undefined' && stickerLibrary.length > 0) ? stickerLibrary : [];
    if (stickers.length === 0) {
        if (typeof showNotification === 'function') {
            showNotification('表情库为空，请先在"高级功能→自定义回复"中添加', 'warning');
        }
        return;
    }
...
window._bv2_stickerBackup = [...(window._bv2_selectedStickers || [])];
    ...
    overlay.id = 'bv2-sticker-picker-overlay';
    ...
    panel.id = 'bv2-sticker-picker-panel';
    panel.style.cssText = `
        background:var(--secondary-bg); border-radius:16px; padding:20px;
    ...
    header.className = 'bv2-sticker-picker-header';
    header.style.cssText = `
        display:flex; justify-content:space-between; align-items:center;
        margin-bottom:16px; font-size:14px; font-weight:600;
        color:var(--text-primary); flex-shrink:0;
    `;
    const selectedCount = window._bv2_selectedStickers?.length || 0;
    header.innerHTML = `
        <span>选择表情包 <span style="font-size:12px;font-weight:400;color:var(--text-secondary);">（${selectedCount}/2）</span></span>
        <button id="bv2-sticker-picker-close" style="
            background:none;border:none;color:var(--text-secondary);
            font-size:22px;cursor:pointer;padding:4px 8px;
            transition:all 0.2s;border-radius:50%;line-height:1;
        " onmouseover="this.style.background='var(--border-color)'" onmouseout="this.style.background='transparent'">×</button>
    `;
    ...
    footer.style.cssText = `
        display:flex; gap:8px; margin-top:16px; flex-shrink:0;
    `;
    const confirmBtn = document.createElement('button');
    confirmBtn.id = 'bv2-sticker-picker-confirm';
    ...
    `;
    confirmBtn.textContent = `确定（${selectedCount}/2）`;
...
// 渲染表情包网格（只更新内容，不重建DOM）
function renderPickerGrid() {
    const grid = document.getElementById('bv2-sticker-picker-grid');
    if (!grid) return;
    ...
    stickers.forEach((stickerUrl) => {
        const isSelected = selected.includes(stickerUrl);
        ...
        `;
        item.innerHTML = `
            <img src="${stickerUrl}" style="
                width:100%; height:100%; object-fit:contain; display:block;
                padding:4px; box-sizing:border-box;
            ">
            ${isSelected ? '<div style="position:absolute;top:4px;right:4px;background:var(--accent-color);color:#fff;border-radius:50%;width:22px;height:22px;display:flex;align-items:center;justify-content:center;font-size:12px;font-weight:700;box-shadow:0 2px 6px rgba(0,0,0,0.2);">✓</div>' : ''}
        `;
...
window._showBoardReplyPopup = function(threadId, replyContent, partnerName, type) {
  ...
  // 区分场景
    ...
    popup.innerHTML = '<style>@keyframes slideUpNotif{from{opacity:0;transform:translateX(-50%) translateY(24px) scale(0.9)}60%{transform:translateX(-50%) translateY(-4px) scale(1.02)}to{opacity:1;transform:translateX(-50%) translateY(0) scale(1)}}</style><div style="display:flex;align-items:center;gap:10px;"><span style="font-size:26px;">💌</span><div><div style="font-size:14px;font-weight:700;color:var(--text-primary);">' + partnerName + '回复了你的留言</div><div style="font-size:11px;color:var(--text-secondary);margin-top:2px;opacity:0.8;">' + preview + '</div></div></div><div style="display:flex;gap:8px;"><button onclick="document.getElementById(\'board-reply-popup\').remove();" style="flex:1;padding:8px 0;border-radius:12px;border:1px solid var(--border-color);background:var(--primary-bg);color:var(--text-secondary);font-size:13px;cursor:pointer;">稍后查看</button><button onclick="window._openBoardReply(\'' + threadId + '\');" style="flex:2;padding:8px 0;border-radius:12px;border:none;background:var(--accent-color);color:#fff;font-size:13px;font-weight:600;cursor:pointer;">立即阅读 ✉</button></div>';
    document.body.appendChild(popup);
...
// ---------- 点击"立即阅读" ----------
window._openBoardReply = function(threadId) {
    var popup = document.getElementById('board-reply-popup');
    if (popup) popup.remove();

    if (window.boardDataV2) {
        var data = window.boardDataV2;
        var allThreads = [].concat(data.myThreads || [], data.partnerThreads || []);
        for (var i = 0; i < allThreads.length; i++) {
            if (allThreads[i].id === threadId) {
    ...
    if (typeof window.renderEnvelopeBoard === 'function') {
        ...
        setTimeout(function() {
            if (typeof window.openDetail === 'function') {
                window.openDetail(threadId, window._bv2_currentView || 'me');
            }
        }, 500);
    }
};

// ---------- 检测新回复并弹窗（核心函数，类似 envelope 的 checkEnvelopeStatus） ----------
window._checkBoardNewReplies = function() {
    var data = window.boardDataV2;
    if (!data) return;
    ...
    for (var i = 0; i < allThreads.length; i++) {
        ...
        if (thread.unread) {
            ...
            if (lastReply && lastReply.sender === 'partner') {
                ...
                var replyText = lastReply.text || (lastReply.image ? '🖼 图片留言' : '');
                window._showBoardReplyPopup(thread.id, replyText, partnerName);
                window._boardNotifiedThreads.add(thread.id);
                hasNewReply = true;
            }
...
// ---------- 增强 checkStatus（类似 envelope 的定时检查） ----------
var _boardOriginalCheckStatus = window.checkStatus;

window.checkStatus = function() {
    // 先执行原有逻辑（生成回复等）
    if (typeof _boardOriginalCheckStatus === 'function') {
        _boardOriginalCheckStatus();
    }
    
    // 然后检查是否有新回复需要弹窗（类似 envelope 的 checkEnvelopeStatus）
    setTimeout(function() {
        try {
            if (typeof window._checkBoardNewReplies === 'function') {
                window._checkBoardNewReplies();
...
// ---------- 页面加载时检查（类似 envelope 的初始化检查） ----------
// 在 loadData 完成后执行
var _boardLoadCheck = function() {
    setTimeout(function() {
        if (typeof window._checkBoardNewReplies === 'function') {
            console.log('[BoardReply] 页面加载，检查未读回复...');
            window._checkBoardNewReplies();
...
// 如果 loadData 已经完成，直接执行；否则等待
if (typeof window.loadEnvelopeData === 'function') {
    // loadData 完成后会触发，我们在 loadData 的 then 里已经调用了
    // 但为了保险，再检查一次
    setTimeout(_boardLoadCheck, 2000);
}

// ---------- 页面可见性变化时检查（手机切回前台，类似 envelope） ----------
document.addEventListener('visibilitychange', function() {
    if (document.visibilityState === 'visible') {
        setTimeout(function() {
            if (typeof window._checkBoardNewReplies === 'function') {
