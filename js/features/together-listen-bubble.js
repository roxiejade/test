...
// 构建两个波形周期
// ============================================================
function buildTwoCycles() {
    var BASELINE_COUNT = 40;
    var result = [];
    for (var i = 0; i < BASELINE_COUNT; i++) {
        result.push({ x: i / BASELINE_COUNT * 0.18, y: baselineY });
    }
    var offset1 = result[result.length - 1].x;
    for (var i = 0; i < points.length; i++) {
        result.push({ x: offset1 + points[i].x * 0.72, y: points[i].y });
    }
    var offsetEnd1 = result[result.length - 1].x;
    for (var i = 0; i < 6; i++) {
...
var gapEnd = gapStart + 0.02;
    ...
    var offset2 = result[result.length - 1].x;
    for (var i = 0; i < points.length; i++) {
        result.push({ x: offset2 + points[i].x * 0.72, y: points[i].y });
    }
    var offsetEnd2 = result[result.length - 1].x;
    for (var i = 0; i < BASELINE_COUNT; i++) {
        result.push({ x: offsetEnd2 + (i / BASELINE_COUNT) * 0.18, y: baselineY });
    }
    return result;
}

var twoCycles = buildTwoCycles();
var xMin = twoCycles[0].x;
var xMax = twoCycles[twoCycles.length - 1].x;
var xRange = xMax - xMin;
for (var i = 0; i < twoCycles.length; i++) {
    twoCycles[i].normX = (twoCycles[i].x - xMin) / xRange;
}

// ============================================================
// 绘制爱心
// ============================================================
function drawHeartOutline(ctx, cx, cy, size, progress, heartX, halfWidth) {
    var s = size;
    ctx.save();
    var steps = 80;
    var pathPoints = [];
    ...
    var litLength = (progress - (heartX - halfWidth)) / (halfWidth * 2);
...
function drawBubbleWave(canvas, progress) {
    ...
    ctx.beginPath();
    for (var i = 0; i < twoCycles.length; i++) {
        var px = pad + twoCycles[i].normX * drawW;
        var py = midY + (twoCycles[i].y / scaleBase) * amp;
    ...
    var glowPoints = [];
    for (var j = 0; j < twoCycles.length; j++) {
        var xVal = twoCycles[j].normX;
        if (xVal >= startX && xVal <= endX) {
            glowPoints.push(twoCycles[j]);
        }
    }
    if (glowPoints.length > 1) {
        ...
        for (var k = 0; k < glowPoints.length; k++) {
            var gx = pad + glowPoints[k].normX * drawW;
        ...
        for (var m = 0; m < glowPoints.length; m++) {
            var hx = pad + glowPoints[m].normX * drawW;
        ...
        for (var n = 0; n < glowPoints.length; n++) {
            var fx = pad + glowPoints[n].normX * drawW;
    ...
    var heartScale = 0.4;
    var rWavePeak = 0.986;
    var heartSize = (rWavePeak / scaleBase) * amp * heartScale;
    var heartCx = pad + 0.5 * drawW;
    var heartCy = midY + (baselineY / scaleBase) * amp;
    var heartHalfWidth = 0.06;
    drawHeartOutline(ctx, heartCx, heartCy, heartSize, progress, 0.5, heartHalfWidth);
}
